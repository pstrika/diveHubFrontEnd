<x-page-template bodyClass='g-sidenav-show  bg-gray-200'>
    <x-auth.navbars.sidebar activePage="Calendars" activeItem="wreckDiving" activeSubitem=""></x-auth.navbars.sidebar>
    
    
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
        <!-- Navbar -->
        <x-auth.navbars.navs.auth pageTitle="Wreck Diving Calendar {{ $currentMonthS}}-{{ $year }}"></x-auth.navbars.navs.auth>
        <!-- End Navbar -->
                ...
                <div class="card p-0 position-relative mt-n7 mx-2 z-index-2">
                    ...
                    <div class="p-0 mt-n4 mx-2 border-radius-lg py-3 pe-1">
                        <div style="float: left;">
                            <h1 class="card-category text-info mx-4 mt-3 text-xl">Wreck Diving Calendar</h1>
                            <h2 class="card-title text-info mx-4 text-xl mt-n2">{{ $currentMonthS }}-{{ $year }}</h2>
                        </div>

                        {{-----------------NAV to next day}} --}}
                        <div class="mt-5" style="float: right;">
                            <a type="button" href="/CalendarWreck/{{ $prevMonthS }}/" class="btn btn-info tex-end">
                                <span class="material-icons" style="font-size :24pt;">keyboard_arrow_left</span>
                            </a>
                            
                            <a type="button" href="/CalendarWreck/{{ $nextMonthS }}/" class="btn btn-info tex-end">
                        ...
                        <table class="table align-items-center mb-1">
                                        ...
                                        <td>
                                            <div class="dropdown">
                                                <select class="btn bg-info dropdown-toggle text-white" type="button" id="filterLoc" data-bs-toggle="dropdown" aria-expanded="false">
                                                    ...
                                                    <option value="STU">stuart</option>
                                ...
                                <table>
                                    <tr><td class="text-start text-sm w-1"> 
                                        ...
                                        <span class="badge badge-md bg-gradient-danger text-white">Rainbow Reef</span>
                                        <span class="badge badge-md bg-gradient-warning text-white">Sea Experience</span>
                                        <span class="badge badge-md bg-gradient-info text-white">Divers Paradise</span>
                    ...
                    <div class="card p-0 position-relative mt-5 mx-0 z-index-2 mb-4">
                        <div class="card-header p-0 mt-n4 mx-3">
                            <div class="bg-gradient-info shadow-info border-radius-xl py-3 pe-1">
                                <h2 class="card-title text-white mx-4">Wreck Dives {{ $currentMonthS }}-{{ $year }}</h2>
                            </div>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive">
    ...
    <script>
        ...
        events: [
            @php
                //Log::debug("Trips to add" . count($trips));
                foreach($trips as $trip) {
                    // fix the ' problem
                    ...
                    if($trip->operatorId == "1" or $trip->operatorId == "21")
                        echo "className: 'bg-gradient-success text-white opId=1 loc=" . substr($trip->tags, 0, 3) . " isAvail=" . (($trip->tripFreeSpots > 0) ? "Y" : "N")  . " level=" . $trip->site[0]->level . "' },";
                    elseif($trip->operatorId == "13")
                        echo "className: 'bg-gradient-danger text-white opId=3 loc=" . substr($trip->tags, 0, 3) . " isAvail=" . (($trip->tripFreeSpots > 0) ? "Y" : "N")  . " level=" . $trip->site[0]->level . "' },";
                    elseif($trip->operatorId == "6")
                        echo "className: 'bg-gradient-warning text-white opId=8 loc=" . substr($trip->tags, 0, 3) . " isAvail=" . (($trip->tripFreeSpots > 0) ? "Y" : "N")  . " level=" . $trip->site[0]->level . "' },";
                    elseif($trip->operatorId == "8")
