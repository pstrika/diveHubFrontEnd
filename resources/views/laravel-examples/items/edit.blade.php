<x-page-template bodyClass='dh-shell bg-gray-200'>
    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
        <!-- Navbar -->
        <x-shell.header title="Item Management" />
        <!-- End Navbar -->
                            ...
                            <form method='POST' action='{{ route('edit.item', $item) }}'
                                ...
                                <div class="avatar avatar-xxl position-relative">
                                    <div class="position-relative preview">
                                        ...
                                        <span class="h-12 w-12 rounded-full overflow-hidden bg-gray-100">
                                            <img src="/storage/{{$item->picture}}" alt="avatar" id="file-ip-1-preview"
                                            ></span>
                                ...
                                <div class="form-group col-12 col-md-6 mt-3">
                                    ...
                                    <select class="col-12 border border-5" name="tags[]"
                                        ...
                                        @foreach ($tags as $tag)
                                        <option value="{{ $tag->id }}"
                                            {{ in_array($tag->id, old('tags', $item->tag->pluck('id')->toArray()) ?: []) ? 'selected' : '' }}>
                                            {{ $tag->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('tags')
                                ...
                                <div class="form-group col-12 col-md-6 mt-3">
                                    ...
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="status" id="archive"
                                            value='archive'
                                            {{ old('status', $item->status) == 'archive' ? ' checked' : '' }}>
                                ...
                                <div class="form-check form-switch col-12 col-md-6 mt-3">
                                    ...
                                    <input class="form-check-input" type="checkbox" id="flexSwitchCheckDefault"
                                        ...
                                        {{ old('homepage', $item->homepage) == 1 ? ' checked' : '' }}>
                                </div>
                                <div class="form-group col-12 col-md-6 mt-3">
                                    ...
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" value="1" name='options[]'
                                            id="flexCheckFirst"
                                            {{ old('options', $item->options) && in_array( '1', old('options', $item->options)) ? ' checked' : '' }}>
                                    ...
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" value="2" name='options[]'
                                            id="flexCheckSecond"
                                            {{ old('options', $item->options) && in_array( '2', old('options', $item->options)) ? ' checked' : '' }}>
                                    ...
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" value="3" name='options[]'
                                            id="flexCheckThird"
                                            {{ old('options', $item->options) && in_array( '3', old('options', $item->options)) ? ' checked' : '' }}>
                                ...
                                <div class="form-group col-12 col-md-6 mt-3">
                                    ...
                                    <input type="date" name="date" class="datetimepicker form-control border border-2 p-2"
                                        id="exampleDate" value="{{ old('date', $item->date) }}">
                                        @error('date')
                                        <p class='text-danger inputerror'>{{ $message }} </p>
                                        @enderror
                                </div>
